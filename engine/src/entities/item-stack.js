...
ItemStack.prototype.hasHeightItem = function () {

  for (let i = 0; i < this.__items.length; i++) {
    if (this.__items[i].hasHeight() && !this.__items[i].isMoveable()) {
      return true;
    }
  }
...
ItemStack.prototype.countHeight = function () {

  /*
   * Function ItemStack.countHeight
   * Returns the number of items on the stack with height
   */
...
ItemStack.prototype.getTeleporterDestination = function() {
  ...
  for(let i = 0; i < this.__items.length; i++) {
    if(this.__items[i].isTeleporter()) {
      return this.__items[i].destination || null;
    }
  }
...
ItemStack.prototype.applyFieldDamage = function (creature) {
   ...
   * Applies field damage to a creature when it steps on the tile
   */

  for (let i = this.__items.length - 1; i >= 0; i--) {
    let proto = this.__items[i].getPrototype();
    if (!proto.isField()) continue;
...
ItemStack.prototype.__applyFieldCondition = function(field, creature) {
  ...
  if (immunityType && creature.damageImmunities && creature.damageImmunities.includes(immunityType)) {
    return;
  }
...
ItemStack.prototype.hasDamagingField = function (creature) {

  /*
   * Function ItemStack.hasDamagingField
   * Returns true if the tile has a damaging field item and the creature is not immune
   */
  ...
  for (let i = this.__items.length - 1; i >= 0; i--) {
    let proto = this.__items[i].getPrototype();
    if (!proto.isField()) continue;
    let field = proto.properties.field;
    let immunityType = immunityMap[field];
    if (immunityType && creature.damageImmunities && creature.damageImmunities.includes(immunityType)) {
      return false;
    }
    return true;
