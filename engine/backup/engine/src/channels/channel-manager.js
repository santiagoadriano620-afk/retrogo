...
const TradeChannel = requireModule("channels/channel-trade");
const RookgaardChannel = requireModule("channels/channel-rookgaard");
...
const ChannelManager = function() {
  ...
  this.__channels.set(CONST.CHANNEL.TRADE, new TradeChannel(CONST.CHANNEL.TRADE, "Trade"));
  this.__channels.set(CONST.CHANNEL.HELP, new GlobalChannel(CONST.CHANNEL.HELP, "Help"));
  this.__channels.set(CONST.CHANNEL.ROOKGAARD, new RookgaardChannel(CONST.CHANNEL.ROOKGAARD, "Rookgaard"));
...
ChannelManager.prototype.handleSendPrivateMessage = function(player, packet) {
  ...
  // Check if target ignores the sender
  if (targetPlayer.ignorelist && targetPlayer.ignorelist.has(player.getProperty(CONST.PROPERTIES.NAME))) {
    return;
  }
