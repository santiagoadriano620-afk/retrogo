...
const ChannelManager = function() {
  ...
  document.getElementById("prev-channel-button").addEventListener("click", this.handleChannelIncrement.bind(this, -1));
  document.getElementById("next-channel-button").addEventListener("click", this.handleChannelIncrement.bind(this, 1));
  ...
  document.getElementById("chat-say-loudness").addEventListener("click", this.__changeLoudness.bind(this));

  // Toggle chat button
  let toggleChatBtn = document.getElementById("toggle-chat-btn");
  if(toggleChatBtn) {
    toggleChatBtn.addEventListener("click", this.toggleInputLock.bind(this));
  }

  // Close channel button
  let closeBtn = document.getElementById("close-channel-button");
  if(closeBtn) {
    closeBtn.addEventListener("click", this.closeCurrentChannel.bind(this));
  }

  // Trade channel prefix
  this.__tradePrefix = null;

  // Trade channel SELL/BUY buttons
  this.__sellBtn = document.getElementById("trade-sell-btn");
  this.__buyBtn = document.getElementById("trade-buy-btn");

  if(this.__sellBtn) {
    this.__sellBtn.addEventListener("click", this.__handleTradeSellClick.bind(this));
  }
  if(this.__buyBtn) {
    this.__buyBtn.addEventListener("click", this.__handleTradeBuyClick.bind(this));
  }

  // Protect SELL/BUY prefix in trade channel
  this.__inputElement.addEventListener("input", this.__ensureTradePrefix.bind(this));

}

ChannelManager.prototype.__voiceEnabled = false;

ChannelManager.prototype.getLoudnessIndex = function() {
...
ChannelManager.prototype.__changeLoudness = function() {
  ...
  var img = document.getElementById("say-mode-icon");

  this.__voiceEnabled = !this.__voiceEnabled;

  if (this.__voiceEnabled) {
    if (img) img.src = "/images/game/console/mic.svg";
    gameClient.interface.voiceManager.enable().then(function () {
      gameClient.interface.voiceManager.startRecording();
    });
  } else {
    if (img) img.src = "/images/game/console/mic-off.svg";
    gameClient.interface.voiceManager.stopRecording();
  }
...
ChannelManager.prototype.toggleInputLock = function() {
  ...
  if(this.__disabled) {
    this.__inputElement.placeholder = "";
  ...
  this.__updateToggleChatVisual();

}

ChannelManager.prototype.__updateToggleChatVisual = function() {

  /*
   * Function ChannelManager.__updateToggleChatVisual
   * Updates the toggle chat button visual based on disabled state
   */
...
ChannelManager.prototype.joinChannel = function(id, name) {
  ...
  gameClient.send(new ChannelJoinPacket(id));
...
ChannelManager.prototype.closeChannel = function(channel) {
  ...
  gameClient.send(new ChannelLeavePacket(channel.id));
...
ChannelManager.prototype.__handlePrivateMessageSend = function(channel, message) {
  ...
  gameClient.send(new ChannelPrivatePacket(channel.name, message));
...
ChannelManager.prototype.handleMessageSend = function() {
  ...
  // Trade channel requires message to start with SELL or BUY
  if(channel.id === CONST.CHANNEL.TRADE) {
    let upper = message.toUpperCase();
    if(!upper.startsWith("SELL") && !upper.startsWith("BUY")) {
      let prefix = this.__tradePrefix || "SELL ";
      this.__inputElement.value = prefix + message;
      message = this.__inputElement.value;
    }
  }

  let loudness = this.getLoudnessIndex();
...
ChannelManager.prototype.__handleTradeSellClick = function() {

  this.__ensureTradeChannelActive();
  this.__tradePrefix = "SELL ";
  this.__inputElement.value = this.__tradePrefix;
  this.__enableInput();

}

ChannelManager.prototype.__handleTradeBuyClick = function() {

  this.__ensureTradeChannelActive();
  this.__tradePrefix = "BUY ";
  this.__inputElement.value = this.__tradePrefix;
  this.__enableInput();

}

ChannelManager.prototype.__enableInput = function() {

  if(this.__disabled) {
    this.__disabled = false;
    this.__inputElement.disabled = false;
    this.__inputElement.placeholder = "";
    this.__inputElement.focus();
    this.__updateToggleChatVisual();
  } else {
...
ChannelManager.prototype.__ensureTradePrefix = function() {

  let channel = this.getActiveChannel();
  if(!channel || channel.id !== CONST.CHANNEL.TRADE || !this.__tradePrefix) {
    return;
  }

  let val = this.__inputElement.value;
  let upper = val.toUpperCase();

  if(!upper.startsWith("SELL") && !upper.startsWith("BUY")) {
    this.__inputElement.value = this.__tradePrefix + val;
  }

}

ChannelManager.prototype.__ensureTradeChannelActive = function() {

  let channel = this.getChannelById(CONST.CHANNEL.TRADE);
  if(channel) {
    this.setActiveChannelElement(channel);
  } else {
    this.joinChannel(CONST.CHANNEL.TRADE, "Trade");
  }

}

ChannelManager.prototype.__updateTradeButtons = function() {

  if (!window.CONST || !window.CONST.CHANNEL) return;
  let channel = this.getActiveChannel();
  if (!channel) return;
  let isTrade = channel.id === CONST.CHANNEL.TRADE;

  if(this.__sellBtn) {
    this.__sellBtn.style.display = isTrade ? "inline-flex" : "none";
  }
  if(this.__buyBtn) {
    this.__buyBtn.style.display = isTrade ? "inline-flex" : "none";
  }
...
ChannelManager.prototype.setActiveChannel = function(index) {
  ...
  // Clear trade prefix when switching channels
  this.__tradePrefix = null;
  ...
  // Update trade buttons visibility
  this.__updateTradeButtons();
