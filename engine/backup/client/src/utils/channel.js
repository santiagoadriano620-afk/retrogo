...
Channel.prototype.lastMessageSelf = function() {
  ...
  let last = this.__contents.filter(x => gameClient.player.name === x.name).last();
...
Channel.prototype.render = function() {
  ...
  this.__contents.forEach(function(message) {
    if (message.name && gameClient.player && gameClient.player.ignorelist && gameClient.player.ignorelist.has(message.name)) {
      return;
    }
