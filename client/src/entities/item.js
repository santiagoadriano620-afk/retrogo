...
Item.prototype.isAmmunition = function() {
  return this.getProjectileType() !== null;
}

Item.prototype.getProjectileType = function() {
  if (!CONST.AMMUNITION) return null;
  for (let type in CONST.AMMUNITION) {
    if (CONST.AMMUNITION[type].indexOf(this.id) !== -1) {
      let map = CONST.AMMO_PROJECTILE_MAP || {};
