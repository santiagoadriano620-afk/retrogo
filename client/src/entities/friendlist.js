...
Friendlist.prototype.__addFriends = function(friends) {
...
Friendlist.prototype.__add = function(friend) {
...
Friendlist.prototype.add = function(friend) {
   ...
   * Adds a single friend to the friend list and re-renders the DOM
   */
...
Friendlist.prototype.addFromServer = function(name, online) {

  if (this.has(name)) {
    return;
  }

  this.__friends.set(name, online || false);
  this.updateDOM();
...
Friendlist.prototype.updateDOM = function() {
  ...
  // Deconstruct data
  let array = Array.from(this.__friends, this.__deconstructMap).filter(this.__showOfflineFilter, this).sort(this.__sortFunction);
