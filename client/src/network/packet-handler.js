...
PacketHandler.prototype.handlePropertyChange = function (packet) {
  ...
  switch (packet.property) {
    case CONST.PROPERTIES.NAME:
      creature.name = packet.value;
      if (creature.characterElement) {
        creature.characterElement.setName(packet.value);
      }
      return;
...
PacketHandler.prototype.handleCondition = function (packet) {
  ...
  if (packet.toggle) {
    ...
    if (packet.cid === ConditionManager.prototype.LIGHT && packet.duration > 0) {
      creature.__lightLevel = packet.duration;
    }
  } else {
    creature.removeCondition(packet.cid);
    if (packet.cid === ConditionManager.prototype.LIGHT) {
      creature.__lightLevel = null;
    }
  }
...
PacketHandler.prototype.handleOracleShow = function (packet) {
  gameClient.interface.modalManager.open("oracle-modal", packet)
}
...
PacketHandler.prototype.handleEmote = function (packet) {
  ...
  gameClient.interface.screenElementManager.createTextElement(
    sourceCreature,
    packet.message,
    packet.color,
    false
...
PacketHandler.prototype.handleChunk = function (chunk) {
  ...
  this.__scheduleNeighbourUpdate();

}

PacketHandler.prototype.__scheduleNeighbourUpdate = function () {

  /*
   * Function PacketHandler.__scheduleNeighbourUpdate
   * Debounces neighbour references to avoid running per chunk during bulk loads
   */

  if (this.__neighbourUpdatePending) {
    return;
  }

  this.__neighbourUpdatePending = true;

  Promise.resolve().then(function () {
    this.__neighbourUpdatePending = false;
...
PacketHandler.prototype.handleRemoveItem = function (packet) {
  ...
  let item = tile.peekItem(packet.index);
...
PacketHandler.prototype.handleBlessingUpdate = function (packet) {

  if (gameClient.player) {
    gameClient.player.blessingBitmask = packet.bitmask;
    gameClient.player.isPremium = packet.isPremium;
    gameClient.__updateBlessingButton();
    let modal = gameClient.interface.modalManager.get("blessing-modal");
    if (modal && modal.element.style.display !== "none") {
      modal.handleOpen();
    }
  }
...
PacketHandler.prototype.clientSideMoveCheck = function (position) {
  ...
  // Allow stepping up to 1 elevation level per move (stairs-like behavior up to 4)
  let currentTile = gameClient.player.getTile();
  let elevationDiff = tile.countElevation() - currentTile.countElevation();
  if(elevationDiff > 1) {
    return true;
  }
...
PacketHandler.prototype.handleCharacterInformation = function (packet) {
  ...
  let gender = packet.sex === 1 ? "She" : "He";

  let vocationName = "nothing";
  switch (packet.vocation) {
    case 0: vocationName = "None"; break;
...
PacketHandler.prototype.handleContainerOpen = function (packet) {
  ...
  // Create the DOM
  container.createDOM(packet.equipped ? packet.title + "[E]" : packet.title, packet.items, packet.id);

  // Register the element with the window manager
  gameClient.interface.windowManager.register(container.window);

  // Read and clear pendingSlot before opening (avoids leak between opens)
  let pendingSlot = gameClient.__pendingContainerOpen;
  gameClient.__pendingContainerOpen = null;

  // Open the container for the player
  gameClient.player.openContainer(container, pendingSlot);
...
PacketHandler.prototype.handlePartyInvite = function (inviterName) {
  gameClient.player.__pendingPartyInvite = inviterName;
  gameClient.interface.channelManager.addConsoleMessage(
    inviterName + " has invited you to join a party.", Interface.prototype.COLORS.LIGHTGREEN);
  gameClient.interface.notificationManager.setServerMessage(
    inviterName + " invites you to join a party. Right-click " + inviterName + " to accept.", Interface.prototype.COLORS.LIGHTGREEN);
};

PacketHandler.prototype.handlePartyInviteResult = function (accepted, name) {
  if (accepted) {
    gameClient.interface.channelManager.addConsoleMessage(
      name + " has joined the party.", Interface.prototype.COLORS.LIGHTGREEN);
  }
};

PacketHandler.prototype.handlePartyLeave = function () {
  if (gameClient.player) {
    gameClient.player.__partyMembers = null;
    gameClient.player.__partyId = null;
    gameClient.interface.channelManager.addConsoleMessage(
      "You have left the party.", Interface.prototype.COLORS.LIGHTGREEN);
  }
  gameClient.interface.windowManager.getWindow("party-window").sync();
};

PacketHandler.prototype.handlePartySkull = function (creatureId, skullType) {
  let creature = gameClient.world.getCreature(creatureId);
  if (creature) {
    creature.skull = skullType;
  }
};

PacketHandler.prototype.handlePartyShield = function (creatureId, shieldType) {
  let creature = gameClient.world.getCreature(creatureId);
  if (creature) {
    creature.shield = shieldType;
  }
};

PacketHandler.prototype.handlePartyData = function (packet) {
  try {
    let data = packet.readPartyData();
    if (gameClient.player) {
      gameClient.player.__partyMembers = data.members;
      gameClient.player.__partyLeaderId = data.leaderId;
      gameClient.player.__partyId = 1;
    }
    gameClient.interface.windowManager.getWindow("party-window").sync();
  } catch (e) {
    console.warn("handlePartyData error:", e);
  }
};

PacketHandler.prototype.handleVipAddResult = function (name, success, online) {
  if (success) {
    gameClient.player.friendlist.addFromServer(name, online);
    gameClient.interface.channelManager.addConsoleMessage(
      name + " has been added to your VIP list.", Interface.prototype.COLORS.LIGHTGREEN);
  } else {
    gameClient.interface.setCancelMessage("Could not add " + name + " to VIP list. Player does not exist.");
  }
};

PacketHandler.prototype.handleIgnoreData = function (packet) {
  let count = packet.readUInt16();
  if (!gameClient.player || !gameClient.player.ignorelist) return;
  gameClient.player.ignorelist.clear();
  for (let i = 0; i < count; i++) {
    gameClient.player.ignorelist.add(packet.readString());
  }
};

PacketHandler.prototype.handleIgnoreAddResult = function (name, success) {
  if (success) {
    gameClient.player.ignorelist.add(name);
    gameClient.interface.channelManager.addConsoleMessage(
      name + " has been added to your ignore list.", Interface.prototype.COLORS.LIGHTGREEN);
  } else {
    gameClient.interface.setCancelMessage("Could not add " + name + " to your ignore list. Player does not exist.");
  }
};

PacketHandler.prototype.handleIgnoreRemoveResult = function (name, success) {
  if (success) {
    gameClient.player.ignorelist.remove(name);
    gameClient.interface.channelManager.addConsoleMessage(
      name + " has been removed from your ignore list.", Interface.prototype.COLORS.LIGHTGREEN);
  } else {
    gameClient.interface.setCancelMessage("Could not remove " + name + " from your ignore list.");
  }
};

PacketHandler.prototype.handleHouseInfo = function (packet) {
  try {
    let data = JSON.parse(packet.readString());
    gameClient.interface.modalManager.open("house-modal", data);
  } catch (e) {
    console.error("Failed to parse house info:", e);
  }
};

PacketHandler.prototype.handleHouseManageInfo = function (packet) {
  try {
    let data = JSON.parse(packet.readString());
    gameClient.interface.modalManager.open("house-manage-modal", data);
  } catch (e) {
    console.error("Failed to parse house manage info:", e);
    gameClient.interface.setCancelMessage("Failed to open house management.");
  }
};

PacketHandler.prototype.handleRentConfirmInfo = function (packet) {
  try {
    let data = JSON.parse(packet.readString());
    gameClient.interface.modalManager.open("confirm-modal", function () {
      gameClient.send(new HouseConfirmRentPacket(data.price));
    });
    let msg = "You are about to set " + data.houseName + " for rent.\n\n";
    msg += "Rent price: " + data.price.toLocaleString() + " gold\n";
    msg += "Weekly base rent: " + data.weeklyRent.toLocaleString() + " gold\n";
    msg += "Maximum allowed: " + data.maxPrice.toLocaleString() + " gold\n\n";
    msg += "Items inside: " + data.itemCount + " will be moved to your depot in backpacks.\n";
    msg += "All players inside will be teleported out.\n";
    msg += "You will not be able to enter for 30 days.\n\n";
    msg += "Make sure you have enough depot space. ";
    msg += "If depot is full, items may be lost.";
    gameClient.interface.setCancelMessage(msg);
  } catch (e) {
    console.error("Failed to parse rent confirm info:", e);
  }
};

PacketHandler.prototype.handleGuildData = function (jsonString) {
  try {
    let data = JSON.parse(jsonString);
    let modal = gameClient.interface.modalManager.get("guild-modal");
    if (modal) {
      modal.updateData(data);
    }
  } catch (e) {
    console.error("Failed to parse guild data:", e);
  }
};

PacketHandler.prototype.handleGuildInviteResult = function (success, message) {
  gameClient.interface.setCancelMessage(message);
};

PacketHandler.prototype.handleTradeRequest = function (packet) {
  let requesterName = packet.readString();
  let modal = gameClient.interface.modalManager.get("trade-modal");
  if (modal) {
    modal.showRequest(requesterName);
  }
};

PacketHandler.prototype.handleTradeStart = function (packet) {
  try {
    let data = JSON.parse(packet.readString());
    gameClient.interface.modalManager.open("trade-modal", data);
  } catch (e) {
    console.error("Failed to parse trade start:", e);
  }
};

PacketHandler.prototype.handleTradeUpdate = function (packet) {
  try {
    let data = JSON.parse(packet.readString());
    let modal = gameClient.interface.modalManager.get("trade-modal");
    if (modal) {
      modal.updateTrade(data);
    }
  } catch (e) {
...
PacketHandler.prototype.handleTradeConfirm = function (packet) {
  try {
    let data = JSON.parse(packet.readString());
    let modal = gameClient.interface.modalManager.get("trade-modal");
    if (modal) {
      modal.updateConfirm(data);
    }
  } catch (e) {
    console.error("Failed to parse trade confirm:", e);
  }
};

PacketHandler.prototype.handleTradeComplete = function () {
  let modal = gameClient.interface.modalManager.get("trade-modal");
  if (modal) {
    modal.tradeComplete();
  }
...
PacketHandler.prototype.handleTradeCancel = function (packet) {
  let reason = packet.readString();
  let modal = gameClient.interface.modalManager.get("trade-modal");
  if (modal) {
    modal.tradeCancelled(reason);
  }
};
...
PacketHandler.prototype.handleBanPanel = function () {
  ...
  function openBanModal() {
    gameClient.interface.modalManager.open("floater-ban-control");
  }
  if (gameClient.interface.modalManager.__modals.hasOwnProperty("floater-ban-control")) {
    return openBanModal();
  }
  ...
  script.onerror = function () {
    console.error("Failed to load ban control modal.");
  };
  document.body.appendChild(script);
};

PacketHandler.prototype.handlePremiumBalanceUpdate = function (balance) {
  if (gameClient.player) {
    gameClient.player.premiumPoints = balance;
  }
  let modal = gameClient.interface.modalManager.get("shop-modal");
  if (modal) {
    modal.updateBalance(balance);
  }
}

PacketHandler.prototype.handleGlobalBoostUpdate = function (expExpiry, lootExpiry, skillsExpiry) {
  if (!gameClient.interface) return;
  gameClient.interface.__globalBoostExp = expExpiry * 1000;
  gameClient.interface.__globalBoostLoot = lootExpiry * 1000;
  gameClient.interface.__globalBoostSkills = skillsExpiry * 1000;
  gameClient.interface.__updateBoostDisplay();
}

PacketHandler.prototype.handleBotPanel = function (packet) {
  function openBotModal() {
    gameClient.interface.modalManager.open("floater-bot-control");
  }
  if (gameClient.interface.modalManager.__modals.hasOwnProperty("floater-bot-control")) {
    return openBotModal();
  }
  let script = document.createElement("script");
  ...
  script.onerror = function () {
    console.error("Failed to load bot control modal.");
  };
  document.body.appendChild(script);
};

PacketHandler.prototype.handleVoiceData = function (packet) {
  let name = packet.readString();
  let length = packet.readUInt16();
  let data = packet.readBuffer(length);
  if (name === gameClient.player.name) return;
  gameClient.interface.voiceManager.playAudio(name, data);
};

PacketHandler.prototype.handleMarketOpenOwner = function (packet) {
  let data = packet.readMarketOpenOwner();
  let modal = gameClient.interface.modalManager.get("market-modal");
  if (modal) {
    modal.openAsOwner(data.shopName, data.items, data.gold, data.retroGold);
  }
};

PacketHandler.prototype.handleMarketOpenBuyer = function (packet) {
  let data = packet.readMarketOpenBuyer();
  let sellerId = null;
  let sellerCreature = null;
  Object.keys(gameClient.world.activeCreatures).some(function (id) {
    let c = gameClient.world.activeCreatures[id];
    let cName = c.getName && c.getName();
    if (cName) {
      let parts = cName.split("\n");
      let actualName = parts.length > 1 ? parts[parts.length - 1] : cName;
      if (actualName === data.sellerName) {
  ...
  let modal = gameClient.interface.modalManager.get("market-modal");
  if (modal) {
    modal.openAsBuyer(data.sellerName, data.shopName, data.items, sellerId, sellerCreature);
  }
};

PacketHandler.prototype.handleMarketBuyResult = function (packet) {
  let data = packet.readMarketBuyResult();
  let modal = gameClient.interface.modalManager.get("market-modal");
  if (modal) {
    modal.handleBuyResult(data.success, data.message);
  }
  if (!data.success) {
    gameClient.interface.setCancelMessage(data.message);
  }
};

PacketHandler.prototype.handleMarketClosed = function (packet) {
  let data = packet.readMarketClosed();
