...
PacketReader.prototype.slice = function (start, end) {
  ...
  return new PacketReader(this.buffer.subarray(start, end));
...
PacketReader.prototype.readProperty = function () {

  let guid = this.readUInt32();
  let property = this.readUInt8();

  // NAME (0) is a string, others are UInt32
  if (property === 0) {
    return { guid, property, value: this.readString() };
  }

  return { guid, property, value: this.readUInt32() };
...
PacketReader.prototype.readTradeOffer = function () {
  ...
  // Read server-provided ownership data (counts per offer, 0 = none)
  let ownedCount = this.readUInt8();
  let owned = [];
  for (let i = 0; i < ownedCount; i++) {
    owned.push(this.readUInt8());
  }
...
PacketReader.prototype.__readChunkTiles = function () {
  ...
  for (let i = 0; i < depth; i++) {
    ...
    for (let j = 0; j < Chunk.prototype.getNumberTilesLayer(); j++) {

      if (ntiles === 0) {
        tiles.push(new Object({
          "id": 0,
          "flags": 0,
          "houseState": 0,
          "items": new Array()
...
PacketReader.prototype.readThing = function (id, count) {

  let thing = new Thing(id, count);

  if (thing.isFluidContainer() || thing.isSplash()) {
    return new FluidThing(id, count);
  }
...
PacketReader.prototype.readBuffer = function (size) {
  ...
  let slice = this.buffer.slice(this.index, this.index + size);
  this.index += size;
...
PacketReader.prototype.readSkills = function () {
...
PacketReader.prototype.readPlayerInfo = function () {
  ...
  let blessingBitmask = this.readUInt8();
  let isPremium = this.readUInt8();
  ...
  return new Object({
    ...
    "blessingBitmask": blessingBitmask,
    "isPremium": isPremium
  });
}
...
PacketReader.prototype.readArray = function () {
...
PacketReader.prototype.discard = function () {
...
PacketReader.prototype.readOracleShow = function () {
  let npcId = this.readUInt32();
  let vocationCount = this.readUInt8();
  let vocations = [];
  for (let i = 0; i < vocationCount; i++) {
    vocations.push({
      id: this.readUInt8(),
      outfitId: this.readUInt8(),
      name: this.readString()
    });
  }
  let townCount = this.readUInt8();
  let towns = [];
...
PacketReader.prototype.readMarketOpenOwner = function () {
  let shopName = this.readString();
  let gold = this.readUInt32();
  let retroGold = this.readUInt32();
  let count = this.readUInt8();
  let items = [];
  for (let i = 0; i < count; i++) {
    items.push({
  ...
  return { shopName, gold, retroGold, items };
}

PacketReader.prototype.readMarketOpenBuyer = function () {
  let sellerName = this.readString();
  let shopName = this.readString();
  let count = this.readUInt8();
  let items = [];
  for (let i = 0; i < count; i++) {
  ...
  return { sellerName, shopName, items };
}

PacketReader.prototype.readMarketBuyResult = function () {
  return {
...
PacketReader.prototype.readMarketClosed = function () {
  return {
    message: this.readString()
  };
}

PacketReader.prototype.readPartyData = function () {
  let count = this.readUInt8();
  let leaderId = this.readUInt32();
  let members = [];
  for (let i = 0; i < count; i++) {
    let id = this.readUInt32();
    let name = this.readString();
    let healthPercent = this.readUInt8();
    let vocation = this.readUInt8();
    let level = this.readUInt16();
    let outfit = this.readOutfit();
    members.push({ id, name, healthPercent, vocation, level, outfit });
  }
