const FriendWindow = function(element) {
  ...
  // Right-click to open context menu (add friend, sort, etc.)
  element.addEventListener("contextmenu", function (event) {
    event.preventDefault();
    event.stopPropagation();
    gameClient.interface.menuManager.open("friend-window-menu", event);
  });
...
FriendWindow.prototype.__createFriendEntry = function(entry) {
   ...
   * Creates a single DOM element for the VIP entry
   */

  let div = document.createElement("div");
  let color = entry.online ? Interface.prototype.COLORS.LIGHTGREEN : Interface.prototype.COLORS.RED;
  ...
  div.addEventListener("dblclick", function () {
    gameClient.interface.channelManager.addPrivateChannel(entry.name);
  });
