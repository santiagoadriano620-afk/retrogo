...
module.exports = function energyStrike() {
  ...
  if (!target || target.id === this.id) {
    world.sendMagicEffect(frontPos, CONST.EFFECT.MAGIC.TELEPORT);
    return 200;
  }

  let damage = CombatFormulas.getMagicDamage(this, 0.35, 0, 0.55, 0);
  let result = CombatFormulas.calculateFinalDamage(this, target, CombatFormulas.COMBAT_TYPES.ENERGY, damage, 0);
  if (result.finalDamage !== 0) {
    target.decreaseHealth(this, Math.abs(result.finalDamage));
  }

  world.sendMagicEffect(target.position, CONST.EFFECT.MAGIC.TELEPORT);
  return 200;
}
